...
def main():
	...
	if not create_new_collection:
		cursor.execute(f"SELECT title FROM koi_collection")
	...
	cursor.execute(f"SELECT username FROM koi_user")
	...
	for item in items:
		...
		for index, attribute_name in enumerate(headers):
			if attribute_name != name_field:
				insert_datum(owner_id, item_id, attribute_name, item[attribute_name], index + 1, "private" if attribute_name in private_fields else "public")
	...
	cursor.execute(f"UPDATE koi_collection SET cached_values=%s, updated_at=%s WHERE id='{collection_id}'", (json.dumps(cached_values), get_current_time()))
